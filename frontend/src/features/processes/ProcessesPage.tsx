...
import { EmptyState, PageHeader, QueryState, Section } from '../../shared/components';
...
export default function ProcessesPage() {
  ...
  // The HR workspace narrows the catalogue to the routes that department owns.
  // The core workflow API does not model a department owner yet — it reports the
  // owning organization — so narrowing is applied only when the data actually names
  // this department. Otherwise the full catalogue is shown, because hiding every
  // route is worse than showing routes the department does not strictly own.
  const catalogue = result.data;
  const departmentOwned = catalogue?.filter((item) => item.owner === department.departmentName);
  const processes = department.isHrWorkspace && departmentOwned?.length ? departmentOwned : catalogue;
  ...
  if (processes.length === 0) return <EmptyState title="Нет определений процессов" text="Для этой организации ещё не опубликован ни один маршрут." />;

  const process = processes.find((item) => item.id === querySelected) ?? processes[0];
  ...
  return <>
    ...
    <div className="process-layout">
      <Section title="Каталог процессов" meta={`${processes.length} определения`}><label className="field-search process-search"><Search size={16} /><input placeholder="Найти процесс" value={search} onChange={(e) => setSearch(e.target.value)} /></label><div className="process-list">{filteredProcesses.map((item) => <button className={process.id === item.id ? 'active' : ''} onClick={() => setSelected(item.id)} key={item.id}><span className={`process-symbol ${item.state}`}><GitBranch size={17} /></span><span><strong>{item.name}</strong><small>v{item.version} · {item.activeInstances} экземпляров</small></span><span className={`state-label state-${item.state}`}>{item.state === 'published' ? 'Опубликован' : item.state === 'draft' ? 'Черновик' : 'Инцидент'}</span><ChevronRight size={16} /></button>)}</div></Section>
