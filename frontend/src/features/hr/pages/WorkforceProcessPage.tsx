...
function ProcessCreateForm({ kind, close }: { kind: ProcessKind; close: () => void }) {
  ...
  const [values, setValues] = useState({ employeeId: '', leaveTypeId: '', startDate: '', endDate: '', reason: '', destination: '', purpose: '', estimatedCost: '0', currency: 'KZT', reasonId: '', requestedDate: '', legalBasis: '' });
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (kind === 'trip') {
        if (!current.data?.id) throw new Error('Не удалось определить сотрудника текущего аккаунта.');
        return workforceProcessesApi.createTrip({ employeeId: current.data.id, destination: values.destination, startDate: values.startDate, endDate: values.endDate, purpose: values.purpose, estimatedCost: Number(values.estimatedCost), currency: values.currency });
      }
    ...
    onSuccess: async () => { await client.invalidateQueries({ queryKey: ['workforce-processes', kind] }); close(); }
  });
  const submit = (event: FormEvent) => { event.preventDefault(); mutation.mutate(); };
  return <Section title={kind === 'termination' ? 'Запустить увольнение' : kind === 'leave' ? 'Новая заявка на отпуск' : 'Новая командировка'} meta="Backend workflow">
    <form onSubmit={submit} className="field-grid">
      {kind === 'termination' && <>
        ...
        <label className="span-two">Причина<select required value={values.reasonId} onChange={(event) => set('reasonId', event.target.value)}><option value="">Выберите основание</option>{(reasons.data ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label>Желаемая дата<input required type="date" value={values.requestedDate} onChange={(event) => set('requestedDate', event.target.value)} /></label>
      ...
      {kind === 'leave' && <>
        <label className="span-two">Тип отпуска<select required value={values.leaveTypeId} onChange={(event) => set('leaveTypeId', event.target.value)}><option value="">Выберите тип</option>{(leaveTypes.data ?? []).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
        <label>Дата начала<input required type="date" value={values.startDate} onChange={(event) => set('startDate', event.target.value)} /></label>
        <label>Дата окончания<input required type="date" value={values.endDate} onChange={(event) => set('endDate', event.target.value)} /></label>
