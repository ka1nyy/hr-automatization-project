...
export default function HrLeavePage() {
  ...
  const employee = useQuery({ queryKey: ['hr', 'employee', 'me'], queryFn: () => hrRepository.getCurrentEmployee(), enabled: !isHr });
  ...
  const createMutation = useMutation({ mutationFn: (input: LeaveRequestForm) => {
    if (!employee.data) throw new Error('HR_CURRENT_EMPLOYEE_NOT_LOADED');
    return hrRepository.createLeaveRequest({ ...input, employeeId: employee.data.id });
  }, onSuccess: async (created) => { setSuccessNumber(created.documentNumber); form.reset(); await queryClient.invalidateQueries({ queryKey: ['hr'] }); } });
