...
const stages: Record<ProcessKind, Array<{ code: string; name: string }>> = {
  ...
  termination: [{ code: 'hr_review', name: 'HR-проверка' }, { code: 'legal_review', name: 'Юридическая проверка' }, { code: 'signature', name: 'Подписание' }, { code: 'registration', name: 'Приказ' }, { code: 'offboarding', name: 'Офбординг' }, { code: 'scheduled', name: 'Дата увольнения' }, { code: 'completed', name: 'Завершено' }]
};
const terminalStatuses = new Set(['approved', 'registered', 'completed', 'rejected', 'cancelled']);
const openStatuses = new Set(['manager_review', 'hr_review', 'finance_review', 'hr_registration', 'legal_review', 'signature', 'registration', 'offboarding', 'scheduled', 'effective', 'returned']);
...
export default function WorkforceProcessDetailsPage({ kind }: { kind: ProcessKind }) {
  ...
  const action = useMutation({
    mutationFn: async (command: ProcessDecision | 'cancel' | 'register-order' | 'schedule' | 'complete' | 'resubmit' | 'create-tasks') => {
      ...
      if (command === 'create-tasks') return workforceProcessesApi.createTerminationTasks(item as TerminationProcess, Object.keys(taskLabels).map((taskType) => ({ taskType, assignedEmployeeId: item.employee_id })));
      if (command === 'schedule') {
  ...
  const reviewStage = kind !== 'termination' || (item ? ['hr_review', 'legal_review', 'signature'].includes(item.status) : false);
  ...
  return <>
    <PageHeader eyebrow={kind === 'leave' ? 'Заявка на отпуск' : kind === 'trip' ? 'Заявка на командировку' : 'Маршрут увольнения'} title={employee?.fullName ?? 'Сотрудник'} description={subtitle} actions={<><Link className="secondary-button" to={pathByKind[kind]}>Назад к очереди</Link>{employee && <Link className="secondary-button" to={`/hr/employees/${employee.id}`}><UserRound size={16} />Профиль</Link>}</>} />
    ...
    <div className="hiring-detail-grid">
      <section className="hiring-detail-main"><header><span><ShieldCheck size={20} /></span><div><small>Текущий статус</small><strong>{processStatusLabels[item.status] ?? item.status}</strong></div></header>
        <div className="hiring-summary-grid"><dl><dt>Сотрудник</dt><dd>{employee?.fullName ?? item.employee_id}</dd></dl><dl><dt>Подразделение</dt><dd>{employee?.department ?? '—'}</dd></dl>{trip && <><dl><dt>Бюджет</dt><dd>{new Intl.NumberFormat('ru-RU').format(trip.estimated_cost)} {trip.currency}</dd></dl><dl><dt>Цель</dt><dd>{trip.purpose}</dd></dl></>}{leave && <><dl><dt>Тип отпуска</dt><dd>{{ annual_paid: 'Ежегодный оплачиваемый отпуск', unpaid: 'Без сохранения заработной платы' }[leaveTypes.data?.find((type) => type.id === leave.leave_type_id)?.code ?? ''] ?? leaveTypes.data?.find((type) => type.id === leave.leave_type_id)?.name ?? 'Отпуск'}</dd></dl><dl><dt>Комментарий</dt><dd>{leave.reason || '—'}</dd></dl></>}{termination && <><dl><dt>Дата</dt><dd>{formatDate(termination.effective_date ?? termination.requested_date, locale)}</dd></dl><dl><dt>Приказ</dt><dd>{termination.order_document_id ? 'Зарегистрирован' : 'Ещё не зарегистрирован'}</dd></dl></>}</div>
        <h3>Маршрут процесса</h3><Timeline kind={kind} status={item.status} />
        {kind === 'termination' && ['offboarding', 'scheduled', 'effective', 'completed'].includes(item.status) && <div className="offboarding-block"><div className="offboarding-heading"><div><small>ЧЕК-ЛИСТ</small><h3>Офбординг</h3></div><span>{(tasks.data ?? []).length - pendingTasks}/{(tasks.data ?? []).length}</span></div>{tasks.isLoading ? <p className="hr-empty-note">Загрузка задач…</p> : (tasks.data ?? []).length ? <div className="offboarding-list">{tasks.data!.map((task) => <TaskRow key={task.id} task={task} pending={taskAction.isPending} canComplete={canCompleteTask(task.task_type)} canWaive={hrPersona} onComplete={() => taskAction.mutate({ task, mode: 'complete' })} onWaive={() => taskAction.mutate({ task, mode: 'waive' })} />)}</div> : hrPersona ? <button className="secondary-button" disabled={action.isPending} onClick={() => action.mutate('create-tasks')}><ClipboardCheck size={16} />Создать стандартный чек-лист</button> : <p className="hr-empty-note">HR формирует чек-лист после регистрации приказа.</p>}</div>}
      </section>
      <aside className="hiring-document-panel"><h3><FileCheck2 size={18} />Действия</h3>
        ...
        {!canAct && !canCancel && !terminalStatuses.has(item.status) && !(item.status === 'returned' && isEmployeeOwner) && <div className="hiring-empty"><strong>Действий на этом этапе нет</strong><p>После решения текущей роли процесс автоматически перейдёт дальше.</p></div>}
      </aside>
    </div>
  </>;
}
