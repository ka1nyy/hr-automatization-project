...
export default function HrEmployeesPage() {
  ...
  return <>
    ...
    <div className="hr-directory-summary"><span><strong>{result.data?.length ?? 0}</strong> сотрудников</span><span><strong>{departments.length}</strong> подразделений</span><span><strong>{(result.data ?? []).filter((employee) => employee.status === 'probation').length}</strong> на испытательном сроке</span><span><strong>{(result.data ?? []).filter((employee) => employee.status === 'on_leave' || employee.status === 'sick_leave').length}</strong> отсутствуют</span></div>
    <div className="register-toolbar hr-toolbar"><label className="field-search"><Search size={16} /><input value={query} onChange={(event) => { const val = event.target.value; setQuery(val); setSearchParams(prev => { if (val) prev.set('query', val); else prev.delete('query'); return prev; }); }} placeholder="ФИО, должность или табельный номер" /></label><select value={department} onChange={(event) => setDepartment(event.target.value)}><option value="all">Все подразделения</option>{departments.map((item) => <option key={item}>{item}</option>)}</select></div>
